...
import com.vihmessenger.vihchatbot.listener.OnActivityTouchListener;
...
//        mSwipeListener = listener;
    ...
    public RecyclerTouchListener setFgFade() {
        if (!fadeViews.contains(fgViewID)) this.fadeViews.add(fgViewID);
        return this;
    }
    ...
    public void openSwipeOptions(int position) {
        if (!swipeable || rView.getChildAt(position) == null || unSwipeableRows.contains(position) || shouldIgnoreAction(position))
            return;
        if (bgWidth < 2) {
            if (act.findViewById(bgViewID) != null) bgWidth = act.findViewById(bgViewID).getWidth();
                ...
                // it's original position (spring effect)
                    ...
                    animateFG(touchedView, Animation.CLOSE, ANIMATION_STANDARD, new OnSwipeListener() {
                        ...
                        public void onSwipeOptionsClosed() {
                            if (tempBgView != null) tempBgView.setVisibility(View.VISIBLE);
                        }
